...
// Definição global para o reCAPTCHA
declare global {
  interface Window {
    grecaptcha: any;
  }
...
export default function Contact() {
  ...
  const [status, setStatus] = useState<"init" | "sending" | "success" | "error">("init");
  ...
  async function getRecaptchaToken() {
    if (!SITE_KEY) throw new Error("Site key do reCAPTCHA não configurada.");
    if (!window.grecaptcha) throw new Error("reCAPTCHA não carregado.");
    ...
    try {
      ...
      const response = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          ...
          telefone: formData.phone, // ENVIANDO TELEFONE
      ...
      setStatus("error");
      setErrorMessage(error.message || "Erro interno no servidor.");
    }
  }
  ...
  return (
    <section id="contact" className="bg-black py-24 px-6 border-t border-white/5 relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-12">
            ...
            <div className="space-y-6">
              <a href="mailto:filipevieira14@gmail.com" className="flex items-center gap-6 group w-fit">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-white group-hover:text-red-600 transition-colors">filipevieira14@gmail.com</p>
                </div>
              </a>
              <a href="https://wa.me/5519991518640" target="_blank" rel="noopener noreferrer" className="flex items-center gap-6 group w-fit">
                <div className="w-14 h-14 bg-[#111] border border-white/10 flex items-center justify-center group-hover:border-red-600 group-hover:bg-red-600 group-hover:text-white transition-all duration-500">
                  <Phone size={24} weight="thin" />
                </div>
                <div>
                  <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">WhatsApp</p>
                  <p className="text-lg font-bold text-white group-hover:text-red-600 transition-colors">+55 19 99151-8640</p>
                </div>
              </a>
            </div>
          </div>
          ...
          <div className="bg-[#0A0A0A] p-8 lg:p-14 relative border border-white/10 shadow-2xl backdrop-blur-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="group space-y-2">
                <label htmlFor="name" className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 group-focus-within:text-red-600 transition-colors">
                  Seu Nome
                </label>
                <input
                  id="name"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="group space-y-2">
                  <label htmlFor="email" className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 group-focus-within:text-red-600 transition-colors">
                    E-mail
                  </label>
                  <input
                    id="email"
                ...
                {/* NOVO CAMPO: TELEFONE */}
                <div className="group space-y-2">
                  <label htmlFor="phone" className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 group-focus-within:text-red-600 transition-colors">
                    WhatsApp / Telefone
                  </label>
                  <input
                    id="phone"
              ...
              <div className="group space-y-2">
                <label htmlFor="message" className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 group-focus-within:text-red-600 transition-colors">
                  Mensagem
                </label>
                <textarea
                  id="message"
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <ArrowRight size={16} weight="bold" className="group-hover:translate-x-2 transition-transform" />
                </button>

                {/* DIREITOS RECAPTCHA (Obrigatório para ocultar o Badge) */}
              ...
              {status === "success" && (
                <p className="text-green-500 text-[10px] font-bold uppercase tracking-widest text-center animate-pulse">✅ Mensagem enviada com sucesso!</p>
              )}
              {status === "error" && (
        ...
        <div className="mt-32 pt-12 border-t border-white/5 relative">
          ...
          >
            <ArrowUp size={20} weight="bold" className="group-hover:-translate-y-1 transition-transform" />
          </button>
          ...
          <div className="flex flex-col md:flex-row justify-between gap-8 items-center">
            ...
            <div className="flex flex-col items-center md:items-end gap-3">
              <span className="text-white/20 text-[8px] font-bold uppercase tracking-[0.3em]">Built by</span>
              <a href="https://kduarte.vercel.app/" target="_blank" rel="noopener noreferrer" className="group relative flex items-center justify-center w-28 h-14 rounded-sm border border-white/5 hover:border-red-600/50 hover:bg-red-600/5">
                <Image src="/k.svg" alt="KDuarte Logo" width={80} height={30} className="opacity-40 invert brightness-200 group-hover:opacity-100 transition-all" />
              </a>
            </div>
          </div>
        </div>
